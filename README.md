...
- names assets as `<os>-<arch>-vX.Y.Z` for versioned releases and additionally `<os>-<arch>-lastest` for the latest release (used by installers)
...
Set `VBUILD_RELEASE_LATEST=1` to also emit `-lastest` assets for the latest release.
